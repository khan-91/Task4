...
//         // --- Morty leaves boxes (Monty Hall step) ---
//         let remainingBoxes = this.morty.leaveBoxes(rickGuess, portalBox);
//         remainingBoxes.sort((a, b) => a - b); // ensure ascending order

//         // Show remaining box info
//         if (this.morty.constructor.name === "LazyMorty") {
//             const otherBox = remainingBoxes.find(b => b !== rickGuess);
//             console.log(`Morty: I'm keeping the box you chose, I mean ${rickGuess}, remaining box ${otherBox}`);
//         } else {
//             const otherBox = remainingBoxes.find(b => b !== rickGuess);
//             console.log(`Morty: I'm keeping the box you chose, I mean ${rickGuess}, and the box ${otherBox}`);
//         }

//         // --- 2nd provably fair random for ClassicMorty only ---
//         let finalBoxToKeep, secret2, key2;
...
//                 `Morty: Rick, enter your number [0,${remainingBoxes.length}) for collaborative randomness:`,
//                 0, remainingBoxes.length
//             );

//             if (rickGuess === portalBox && remainingBoxes.length > 1) {
//                 const finalIndex = fairRandom2.computeFinalValue(rickValue2);
//                 finalBoxToKeep = remainingBoxes[finalIndex];
//                 secret2 = fairRandom2.mortyValue;
//             } else {
//                 finalBoxToKeep = remainingBoxes.find(b => b === portalBox);
//                 const keepIndex = remainingBoxes.indexOf(finalBoxToKeep);
//                 secret2 = (keepIndex - rickValue2 + remainingBoxes.length) % remainingBoxes.length;
//             }
//             key2 = fairRandom2.key.toString("hex");
//         } else {
//             // LazyMorty does not use 2nd random
//             finalBoxToKeep = remainingBoxes.find(b => b !== rickGuess) || rickGuess;
//         }
...
//         const finalChoice = stayOrSwitch === 0 ? finalBoxToKeep : rickGuess;
...
//         if (this.morty.constructor.name === "ClassicMorty") {
//             console.log(`Morty: 2nd secret = ${secret2}, KEY2=${key2}`);
//             let display2 = (this.morty.constructor.name === "ClassicMorty") ? (rickValue1 + secret2) % remainingBoxes.length : null;
//             console.log(`Morty: 2nd fair number = (${rickValue1} + ${secret2}) % ${remainingBoxes.length} = ${display2} (for display)`);
//         }
